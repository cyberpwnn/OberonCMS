...
public class WorldDeadTreeGenerator extends WorldGenTrees
{
	...
	public WorldDeadTreeGenerator(boolean update, int minHeight, int metaWood, int metaLeaves, boolean vines)
	{
		...
		for(int interval = 0; interval < 1; interval++)
		{
			...
			if(y >= 1 && y + l + 1 <= 256)
			{
				...
				else
				{
					...
					boolean isSoil = block2.equals(Content.Block.ARID_SAND);
					if(isSoil && y < 256 - l - 1)
					{
							...
							for(i2 = x - l1; i2 <= x + l1; ++i2)
							{
								...
								for(int k2 = z - l1; k2 <= z + l1; ++k2)
								{
									...
									if(Math.abs(j2) != l1 || Math.abs(l2) != l1 || random.nextInt(2) != 0 && i3 != 0)
									{
										...
										{
											this.setBlockAndNotifyAdequately(world, i2, k1, k2, Blocks.air, this.metaLeaves);
										}
									}
								}
							}
						...
						for(k1 = 0; k1 < l; ++k1)
						{
							...
							{
								this.setBlockAndNotifyAdequately(world, x, y + k1, z, Content.Block.LOG_ARID, this.metaWood);
